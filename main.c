...
//each cell is a key followed by a value (serialized row)
//Leaf node body layout:
#define LEAF_NODE_KEY_SIZE sizeof(uint32_t)
#define LEAF_NODE_KEY_OFFSET 0
#define LEAF_NODE_VALUE_SIZE ROW_SIZE
#define LEAF_NODE_VALUE_OFFSET (LEAF_NODE_KEY_SIZE + LEAF_NODE_KEY_OFFSET)
...
// table object requests for pages through pager
typedef struct{
    int file_descriptor;
...
If key is not present, return the
...
//binary search to find the child that 
//should contain the given key
Cursor* internal_node_find(Table* table, uint32_t page_num, uint32_t key){
...
//printing the tree with a recursive function
//recursive function that takes any node, then prints it and its children. 
...
//go onto the end of the db file
uint32_t get_unused_page_num(Pager* pager){
	return pager->num_pages;
}

//to keep the tree balanced, evenly
//distribute cells between two new nodes
//if a leaf node holds N cells,
//during a split distribute N+1 cells 
//between two nodes.
//here, left node gets one more cell if N+1 is odd
...
void create_new_root(Table* table, uint32_t right_child_page_num){
	...
	//inititalize root page as new internal node with two children
...
//db_open() calls pager_open() which opens db file
//and keeps track of its size
Pager* pager_open(const char* filename){
...
// get_page() has logic to handle cache miss
void* get_page(Pager* pager, uint32_t page_num){
    ...
    if(pager->pages[page_num]==NULL){
        ...
        if(page_num<=num_pages){
            ...
            long long bytes_read = read(pager->file_descriptor, page, PAGE_SIZE);
...
// Dereference Operator * is used to access value at the ptr address
// Arrow Operator ->
// struct Point *p = &pt;
// printf("%d", p->x);  // 3 — same as (*p).x, but cleaner
PrepareResult prepare_insert(InputBuffer* input_buffer, Statement* statement){
    statement->type = STATEMENT_INSERT;
    //calling strtok on input buffer breaks it into substrings
    //by inserting a null char whenever it reaches a space.
...
// search table for correct place to insert and insert there
// if key already exists there, then return error
